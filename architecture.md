...
- **WorkManager**: `FeedRefreshWorker` s'occupe des synchronisations en arrière-plan. Il est formellement programmé dans `MainActivity` pour tourner chaque heure, avec des contraintes strictes de réseau (connecté) et de batterie (non faible). Il déclenche une notification Push Android Native (`NotificationCompat`) lors de la découverte de nouveaux épisodes, via un `NotificationChannel` dédié.
...
- **Affichage & Design**: Palette `DeepCharcoal` avec accents verts (`#1DB954`). La navigation s'effectue via un `HorizontalPager` à 3 pages (Abonnements, Flux, Recherche) qui remplace le tiroir de navigation standard (hamburger menu).
- **Interactions**: Les `EpisodeCard` supportent le "Long Press" (via `Modifier.combinedClickable`) pour faire apparaître un menu contextuel de gestion ("Rétablir au Début", "Marquer Comme Entendu"). Les cartes affichent la durée restante colorée. Un `MiniPlayer` dynamique persiste à l'écran.
